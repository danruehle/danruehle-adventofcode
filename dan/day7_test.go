...
var day7Input = `mirrored beige bags contain 1 drab brown bag, 3 dotted crimson bags.
...
func parseBagType(s string) BagType {
	const contain = " bags contain "
	containIndex := strings.Index(s, contain)
	color := s[0:containIndex]
	remaining := s[containIndex+len(contain):]
	var others map[string]int
	if remaining != "no other bags." {
		others = make(map[string]int)
		remaining = strings.ReplaceAll(remaining, ".", "")
		remaining = strings.ReplaceAll(remaining, " bags", "")
		remaining = strings.ReplaceAll(remaining, " bag", "")
		rules := strings.Split(remaining, ", ")
		for _, r := range rules {
			count, err := strconv.Atoi(r[0:1])
			if err != nil {
				panic(fmt.Sprintf("Failed to get count for '%s'", r))
			}
			others[r[2:]] = count
		}
	}
	return BagType{color, others, s}
}

func TestParseBagType(t *testing.T) {
	type td struct {
		s  string
		bt BagType
	}
	tests := []td{
		{"light red bags contain 1 bright white bag, 2 muted yellow bags.", BagType{"light red", map[string]int{"bright white": 1, "muted yellow": 2}, ""}},
		{"dark orange bags contain 3 bright white bags, 4 muted yellow bags.", BagType{"dark orange", map[string]int{"bright white": 3, "muted yellow": 4}, ""}},
		{"bright white bags contain 1 shiny gold bag.", BagType{"bright white", map[string]int{"shiny gold": 1}, ""}},
		{"faded blue bags contain no other bags.", BagType{"faded blue", nil, ""}},
	}
	for i, test := range tests {
		test.bt.Source = test.s
		assert.Equal(t, test.bt, parseBagType(test.s), "Parsed bag incorrectly - i: %d s: %#v, type: %#v", i, test.s, test.bt)
	}
}

type BagTypes map[string]BagType

func parseBagTypes(s string) BagTypes {
	bagTypes := make(BagTypes)
	for _, b := range strings.Split(s, "\n") {
		bagType := parseBagType(b)
		bagTypes[bagType.Color] = bagType
	}
	return bagTypes
}

func (bt BagType) canContain(color string, bagTypes BagTypes) bool {
	if _, ok := bt.Others[color]; ok {
		return true
	}
	for c := range bt.Others {
		if obt, ok := bagTypes[c]; ok && obt.canContain(color, bagTypes) {
			return true
		}
	}
	return false
...
func TestDay7a(t *testing.T) {
	bagTypes := parseBagTypes(day7Input)
	var count int
	for _, bt := range bagTypes {
		if bt.canContain("shiny gold", bagTypes) {
			count++
		}
	}
	t.Logf("Bag types that can contain shiny gold bag: %d", count)
}

func (bt BagType) mustContain(bagTypes BagTypes) int {
	var count int
	for s, c := range bt.Others {
		count += c
		if obt, ok := bagTypes[s]; ok {
			count += obt.mustContain(bagTypes) * c
		}
	}
	return count
}
...
func TestDay7b(t *testing.T) {
	bagTypes := parseBagTypes(day7Input)
	bt := bagTypes["shiny gold"]
	count := bt.mustContain(bagTypes)
	t.Logf("Shiny gold bags must contain %d other bags", count)
}
