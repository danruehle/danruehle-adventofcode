...
func parsePassport(lines []string) (passport, []string) {
	p := make(passport)
...
func (p passport) missing(f string) {
	p[f] += "---missing---"
}

func (p passport) invalid(f string) {
	p[f] += "---invalid"
}
...
func TestDay4a(t *testing.T) {
	...
	for i, p := range passports {
		valid := p.isValidA()
		t.Logf("Passport %d: Valid: %v - %#v", i, valid, p)
		if valid {
			totalValid++
		}
	}
	t.Logf("Total valid passports: %d", totalValid)
}
