...
func countQuestionsA(s string) int {
...
func TestCountQuestionsA(t *testing.T) {
	type td struct {
		s string
		c int
	}
	...
	for i, test := range tests {
		assert.Equal(t, test.c, countQuestionsA(test.s), "Question count wrong for test %d - %#v", i, test)
	}
}

func countQuestionsB(s string) int {
	s = strings.ReplaceAll(s, " ", "")
...
func TestCountQuestionsB(t *testing.T) {
	type td struct {
		s string
		c int
	}
	...
	for i, test := range tests {
		assert.Equal(t, test.c, countQuestionsB(test.s), "Question count wrong for test %d - %#v", i, test)
	}
}
...
func TestDay6a(t *testing.T) {
	var count int
	for _, s := range getGroups(day6Input) {
		count += countQuestionsA(s)
	}
