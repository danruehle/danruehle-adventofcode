...
func passorfaila(input string) bool {
	...
	count := strings.Count(pieces[3], pieces[2])
	pass := count >= min && count <= max
	fmt.Printf("pieces: %#v pass: %v\n", pieces, pass)
	return pass
}
func TestDay2a(t *testing.T) {
	var passed int
	for _, password := range passwords {
		if passorfaila(password) {
			passed++
		}
	}
...
func passorfailb(input string) bool {
	input = strings.Replace(input, ": ", "-", 1)
	input = strings.Replace(input, " ", "-", 1)
	pieces := strings.Split(input, "-")
	...
	second, err := strconv.Atoi(pieces[1])
	if err != nil {
		panic(err)
	}
	var password string = pieces[3]
	left := password[first-1 : first]
	right := password[second-1 : second]

	pass := (left == pieces[2]) != (right == pieces[2])
	fmt.Printf("pieces: %#v pass: %v\n", pieces, pass)
	return pass
}
